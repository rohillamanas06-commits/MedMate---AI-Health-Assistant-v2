...
def google_callback():
    ...
    try:
        ...
        print(f"🔐 Google OAuth callback: email={email}, name={name}")
        ...
        if not user:
            ...
            if picture:
                ...
                import requests
                response = requests.get(picture, timeout=10)
                if response.status_code == 200:
                    from werkzeug.utils import secure_filename
                    filename = secure_filename(f"profile_{user.id}_{datetime.now().timestamp()}.jpg")
            ...
            print(f"✅ Created new user from Google: {username}")
        else:
        ...
        print(f"✅ Google login successful: ID={user.id}, username={user.username}")
